...
const blur = async function (req, res, next) {
  if (req.file) {
    const image = await Jimp.read(req.file.path);
    await image.gaussian(5).write(req.file.path);
    next();
  }
};

const invert = async function (req, res, next) {
  if (req.file) {
    const image = await Jimp.read(req.file.path);
    await image.gaussian(15).write(req.file.path);
    next();
  }
};
